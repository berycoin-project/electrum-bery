...
import electrum_bery as electrum
from electrum_bery.bitcoin import TYPE_ADDRESS
from electrum_bery import WalletStorage, Wallet
from electrum_bery_gui.kivy.i18n import _
from electrum_bery.paymentrequest import InvoiceStore
from electrum_bery.util import profiler, InvalidPassword
from electrum_bery.plugins import run_hook
from electrum_bery.util import format_satoshis, format_satoshis_plain
from electrum_bery.paymentrequest import PR_UNPAID, PR_PAID, PR_UNKNOWN, PR_EXPIRED
...
#Factory.register('InstallWizard', module='electrum_bery_gui.kivy.uix.dialogs.installwizard')
#Factory.register('InfoBubble', module='electrum_bery_gui.kivy.uix.dialogs')
#Factory.register('OutputList', module='electrum_bery_gui.kivy.uix.dialogs')
...
from electrum_bery.util import base_units
...
class ElectrumWindow(App):
    ...
    def _get_bu(self):
        return self.electrum_config.get('base_unit', 'BERY')
    ...
    def on_qr(self, data):
        from electrum_bery.bitcoin import base_decode, is_address
        data = data.strip()
        if is_address(data):
            self.set_URI(data)
            return
        if data.startswith('berycoin:'):
            self.set_URI(data)
            return
        # try to decode transaction
        from electrum_bery.transaction import Transaction
        from electrum_bery.util import bh2u
        try:
    ...
    def show_pr_details(self, req, status, is_invoice):
        from electrum_bery.util import format_time
    ...
    def init_ui(self):
        ...
        Factory.register('AnimatedPopup',
                         module='electrum_bery_gui.kivy.uix.dialogs')
        Factory.register('QRCodeWidget',
                         module='electrum_bery_gui.kivy.uix.qrcodewidget')

        # preload widgets. Remove this if you want to load the widgets on demand
        #Cache.append('electrum_bery_widgets', 'AnimatedPopup', Factory.AnimatedPopup())
        #Cache.append('electrum_bery_widgets', 'QRCodeWidget', Factory.QRCodeWidget())
        ...
        self.icon = "icons/electrum-bery.png"
    ...
    def notify(self, message):
        try:
            ...
            icon = (os.path.dirname(os.path.realpath(__file__))
                    + '/../../' + self.icon)
            notification.notify('Electrum-BERY', message,
                            app_icon=icon, app_name='Electrum-BERY')
        except ImportError:
