...
from electrum_bery.util import FileImportFailed, FileExportFailed
from electrum_bery.paymentrequest import PR_UNPAID, PR_PAID, PR_EXPIRED
...
def export_meta_gui(electrum_window, title, exporter):
    ...
    filename = electrum_window.getSaveFileName(_("Select file to save your {}").format(title),
                                               'electrum-bery_{}.json'.format(title), filter_)
    if not filename:
        return
    try:
        exporter(filename)
