...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-BERY"
  !define PRODUCT_WEB_SITE "https://github.com/pooler/electrum-bery"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-bery-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-bery\icons\electrum.ico"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bery-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bery-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-bery-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-bery-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-bery-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\berycoin" "" "URL:berycoin Protocol"
  WriteRegStr HKCU "Software\Classes\berycoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\berycoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\berycoin\shell\open\command" "" "$\"$INSTDIR\electrum-bery-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\berycoin"
