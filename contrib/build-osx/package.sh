...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-BERY.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-bery-macos
	cd /tmp/electrum-bery-macos
...
rm -rf /tmp/electrum-bery-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-bery-macos/image/
cp -r $1 /tmp/electrum-bery-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-BERY_uncompressed.dmg \
    /tmp/electrum-bery-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-BERY_uncompressed.dmg electrum-bery-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-BERY_uncompressed.dmg

echo "Done."
md5sum electrum-bery-$VERSION.dmg
